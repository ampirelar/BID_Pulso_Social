...
eam_pers <- eam_pers %>% .[ ,-(2:7)] %>% .[ ,-(3:6)] %>% mutate(time = "2019") %>% janitor::clean_names()
...
eam_pers$departamentos[eam_pers$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2019 <- eam_pers %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2019") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
...
eam_2018 <- eam2018 %>% .[ -c(1:5),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>%
  .[ ,-(2:7)] %>% .[ ,-(3:6)] %>% mutate(time = "2018") %>% janitor::clean_names()
...
eam_2018$departamentos[eam_2018$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2018 <- eam_2018 %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2018") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2017 <- eam2017 %>% .[ -c(1:6),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>%
  .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2017") %>% janitor::clean_names()
...
eam_2017$departamentos[eam_2017$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2017 <- eam_2017 %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2017") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2016 <- eam2016 %>% .[ -c(1:5),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>%
   .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2016") %>% janitor::clean_names()
...
eam_2016$departamentos[eam_2016$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2016 <- eam_2016 %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2016") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2015 <- eam2015 %>% .[ -c(1:6),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>% 
   .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2015") %>% janitor::clean_names()
...
eam_2015$departamentos[eam_2015$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2015 <- eam_2015 %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2015") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2014 <- eam2014 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>% 
  .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2014") %>% janitor::clean_names()
...
eam_2014$departamentos[eam_2014$departamentos == "Bogot·"] <- "Bogot· D.C."

data_eam_2014 <- eam_2014 %>% filter(departamentos %in% nom_dpto$nivel_label) %>% mutate(id_data = "4", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2014") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2013 <- eam2013 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1:3),] %>%
  .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2013") %>% janitor::clean_names()
...
eam_2013$departamento[eam_2013$departamento == "Bogot·"] <- "Bogot· D.C."
eam_2013$departamento[eam_2013$departamento == "La guajira"] <- "La Guajira"
eam_2013$departamento[eam_2013$departamento == "San andres"] <- "San AndrÈs"

data_eam_2013 <- eam_2013 %>% filter(departamento %in% nom_dpto$nivel_label) %>% mutate(id_data = "3", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2013") %>% 
  rename(nivel_label = departamento, value = produccion_bruta_g) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2012 <- eam2012 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[ ,-(9:12)] %>% 
  .[ -c(1:3),] %>% .[ ,-(2:7)] %>% janitor::clean_names() %>% .[-(2),] %>% mutate(time = "2012")
...
eam_2012$departamento[eam_2012$departamento == "Bogot·, D.C."] <- "Bogot· D.C."
eam_2012$departamento[eam_2012$departamento == "La guajira"] <- "La Guajira"
eam_2012$departamento[eam_2012$departamento == "San andres"] <- "San AndrÈs"
eam_2012$departamento[eam_2012$departamento == "Bolivar"] <- "BolÌvar"
eam_2012$departamento[eam_2012$departamento == "Cordoba"] <- "CÛrdoba"

data_eam_2012 <- eam_2012 %>% filter(departamento %in% nom_dpto$nivel_label) %>% mutate(id_data = "3", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2012") %>% 
  rename(nivel_label = departamento, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2011 <- eam2011 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[ ,-(9:12)] %>% 
  .[ -c(1:3),] %>% .[ -c(2),] %>% .[ ,-(2:7)] %>% janitor::clean_names() %>% mutate(time = "2011")


eam_2011$departamento [eam_2011$departamento == "TOTAL NACIONAL"] <- "Nacional"

nom_dpto <- nom_dpto %>% mutate (nivel_label = chartr("¡…Õ”⁄", "AEIOU", toupper(nom_dpto$nivel_label)))
...
eam_2011$departamento[eam_2011$departamento == "NARI#O"] <- "NARI—O"

data_eam_2011 <- eam_2011 %>% filter(departamento %in% nom_dpto$nivel_label) %>% mutate(id_data = "3", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2011") %>% 
  rename(nivel_label = departamento, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2010 <- eam2010 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[ -c(1:3),] %>% 
  .[ -c(2),] %>% .[ ,-(9:13)] %>% .[ ,-(2:7)] %>% janitor::clean_names() %>% mutate(time = "2010")
...
eam_2010$departamento[eam_2010$departamento == "NARI#O"] <- "NARI—O"
...
data_eam_2010 <- eam_2010 %>% filter(departamento %in% nom_dpto$nivel_label) %>% 
  mutate(id_data = "3", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2010") %>% 
  rename(nivel_label = departamento, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2009 <- eam2009 %>% .[ -c(1:3),] %>% janitor::row_to_names(row_number = 1) %>% .[ ,-(9:13)] %>% 
...
eam_2009$departamento[eam_2009$departamento == "Bogot·"] <- "Bogot· D.C."
...
data_eam_2009 <- eam_2009 %>% filter(departamento %in% nom_dpto$nivel_label) %>% 
  mutate(id_data = "3", variable = "produccion", id_nivel = "dpto", value_label = "ProducciÛn bruta", id_time = "1", time = "2009") %>% 
  rename(nivel_label = departamento, value = produccion) %>% left_join(nom_dpto, by = "nivel_label") %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2013 <- eam_2013 %>% rename( departamentos = "departamento") %>% rename( produccion = "produccion_bruta_g")
...
data_eam_nac <- eam_nac %>% mutate(id_data = "3", variable = "produccion", id_nivel = "nacional", 
                                   value_label = "ProducciÛn bruta", id_time = "1", nivel_value = "1") %>% 
  rename(nivel_label = departamentos, value = produccion) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

write_csv(data_eam_nac, glue("{datos}/base_produccion_col_2009-2019.csv"))
...
area_m <- data_frame( "area" = c("Barranquilla", "Bogot·", "Bucaramanga", "Cali", "Cartagena", "Cauca", "C˙cuta", "Manizales", "MedellÌn", "Pereira", "Valle", "Resto", "Total"),
                      "nivel_value" = c("Barranquilla_A.M.", "Bogot·_D.C.", "Bucaramanga_A.M.", "Cali_A.M.", "Cartagena", "Cauca", "C˙cuta_A.M.", "Manizales_A.M.", "MedellÌn_A.M.", "Pereira_A.M.", "Valledupar_A.M.", "Resto_del_paÌs", "Total_ciudades_y_A.M."))

eam_pers <- eam2019 %>% .[ -c(1:5),] %>% janitor::row_to_names(row_number = 1) %>% .[-c(1),] %>%
  .[ ,-(9:12)] %>% .[ ,-(2:7)] %>% mutate(time = "2019") 

eam_pers <- eam_pers %>% mutate(area = gsub("-","", `¡rea Metropolitana`)) %>% filter(ProducciÛn > 0)
eam_pers <- eam_pers %>% mutate(area = word(`¡rea Metropolitana`, 1))

data_eam_2019 <- eam_pers %>% mutate(id_data = "4", variable = "produccion", id_nivel = "area_metropolitana", 
                                     value_label = "ProducciÛn bruta", id_time = "1") %>% left_join(area_m, by = "area") %>%
  rename(nivel_label = `¡rea Metropolitana`, value = ProducciÛn) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2018 <- eam_2018 %>% mutate(time = "2018") %>% filter(ProducciÛn > 0)
...
eam_2017 <- eam_2017 %>% mutate(time = "2017") %>% filter(ProducciÛn > 0)
...
data_eam_2017 <- eam_2017 %>% mutate(id_data = "4", variable = "produccion", id_nivel = "area_metropolitana", 
                                     value_label = "ProducciÛn bruta", id_time = "1") %>% left_join(area_m, by = "area") %>%
  rename(nivel_label = `Area Metropolitana`, value = ProducciÛn) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
eam_2016 <- eam_2016 %>% mutate(time = "2016") %>% filter(ProducciÛn > 0)
...
eam_2015 <- eam_2015 %>% mutate(time = "2015") %>% filter(ProducciÛn > 0)
...
eam_2013 <- eam_2013 %>% filter( `PRODUCCI”N BRUTA (g)` > 0) %>% janitor::clean_names()
...
area_m <- area_m %>% mutate (area = chartr("¡…Õ”⁄", "AEIOU", toupper(area_m$area)))
...
eam_2010 <- eam_2010 %>% filter( ProducciÛn > 0) %>% janitor::clean_names()
...
eam_2009 <- eam_2009 %>% filter( ProducciÛn > 0) %>% janitor::clean_names()
