...
# Leer base original
monetaria <- read_excel(glue("{datos_ori}/anexo_pobreza_monetaria_20_departamento.xls"), sheet = "LP Act.Met.") %>%
...
monetaria_dpto <- monetaria %>% 
  ...
  mutate(id_data = 13, variable = "linea_pobreza", value_label = "Línea de pobreza monetaria", 
...
# Leer base original
monetaria <- read_excel(glue("{datos_ori}/anexo_pobreza_monetaria_20_nacional.xls"), sheet = "Ingpc_LI_LP Act.Met.") %>%
  drop_na(`...3`) %>% janitor::row_to_names(row_number = 1) 

# Tomamos solo pobreza monetaria (no incluimos cuadro de pobreza extrema)
colnames(monetaria)[1] <- "zona"
monetaria <- monetaria[which(monetaria$zona == "Armenia")[1]:which(monetaria$zona == "Otras cabeceras")[2], ]
...
monetaria_am <- monetaria %>% 
  ...
  mutate(id_data = 13, nivel_value = glue("{nivel_label}_pobreza"), variable = "linea_pobreza", 
         value_label = "Línea de pobreza monetaria", id_nivel = "area_metropolitana", 
...
# Leer base original
extrema <- read_excel(glue("{datos_ori}/anexo_pobreza_monetaria_20_departamento.xls"), sheet = "LPE Act.Met.") %>%
  drop_na(`...3`) %>% janitor::row_to_names(row_number = 1) 
...
extrema_dpto <- extrema %>% 
  left_join(nom_dpto, by = "nivel_label") %>%
  mutate(id_data = 13, variable = "linea_pobreza_extr", value_label = "Línea de pobreza monetaria extrema", 
         id_nivel = "dpto", id_time = 1, time = year, value = pobreza) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value) %>%
  drop_na(nivel_value)
...
# Leer base original
extrema <- read_excel(glue("{datos_ori}/anexo_pobreza_monetaria_20_nacional.xls"), sheet = "Ingpc_LI_LP Act.Met.") %>%
  drop_na(`...3`) 

# colnames(extrema)[1] <- "zona"
extrema <- extrema %>% janitor::row_to_names(row_number = 1) %>% janitor::clean_names()
colnames(extrema)[1] <- "zona"

# Tomamos solo pobreza extrema (no incluimos cuadro de pobreza monetaria)
extrema <- extrema[which(extrema$zona == "Armenia")[2]:which(extrema$zona == "Otras cabeceras")[3], ]
...
extrema <- extrema %>% 
  ...
  rename_at(vars(contains("x")), funs(gsub("x", "", .))) %>%
  rename_at(vars(contains("20")), funs(paste0("year_", .))) %>%
  mutate(year_2012 = as.numeric(year_2012)) %>%
  pivot_longer(cols = starts_with("year"), names_to = "year", values_to = "pobreza") %>%
  mutate(year = as.numeric(gsub("year_", "", year))) %>%
  rename(nivel_label = zona)
...
extrema_am <- extrema %>% 
  filter(nivel_label != "Nacional" & !(nivel_label %in% zonas)) %>%
  mutate(id_data = 13, nivel_value = glue("{nivel_label}_extrema"), variable = "linea_pobreza_extr", 
         value_label = "Línea de pobreza extrema", id_nivel = "area_metropolitana", 
         id_time = 1, time = year, value = pobreza, nivel_value = gsub(" ", "_", nivel_value)) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)
...
extrema_zonas <- extrema %>% 
  filter(nivel_label %in% zonas) %>%
  mutate(id_data = 13, variable = "linea_pobreza_extr", value_label = "Línea de pobreza extrema", 
