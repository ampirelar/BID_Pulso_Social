...
def_col <- lapply(years, function(x){
  ...
  data <- read_excel(glue("{datos_ori}/{x}/Mortalidad_no_fetal/{lista}"), sheet = "00") %>%
    ...
    mutate(cod_enf = as.character(substr(na, 1, 3)), time = x)%>% filter(cod_enf == "511")
...
nac <- def_col %>% mutate(id_data = 19, variable = "suicidios",
...
def_col19 <- read_csv(glue("{datos}/suicidios_col_2019-2020.csv"))
nac <- rbind (nac, def_col19)

nac <- nac %>% left_join(tot, by = "time") %>% mutate(value = (as.numeric(value)/poblacion)*100000) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

write_csv(nac, glue("{datos}/base_suicidios_col_2010-2020.csv"))

rm(nac, def_col19)

# Suicidios - género----

nac <- def_col %>% pivot_longer(cols = starts_with("gen_"), names_to = "gen", values_to = "suicidios") %>%
  mutate(id_data = 19, variable = "suicidios", gen = as.numeric(gsub("gen_", "", gen)),
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         id_nivel = "nacional_gen", id_time = 1, nivel_value = glue("1_{gen}"), value = suicidios) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_col19 <- read_csv(glue("{datos}/suicidios_genero_col_2019-2020.csv"))
nac <- rbind (nac, def_col19)

nac <- nac %>% left_join(tot, by = "time") %>% mutate(value = (as.numeric(value)/poblacion)*100000) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

write_csv(nac, glue("{datos}/base_suicidios_genero_col_2010-2020.csv"))
...
def_col <- def_col %>% mutate(id_data = 19, variable = "suicidios",
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         id_nivel = "nacional_edad", id_time = 1, nivel_value = glue("1_{edad}"), value = suicidios) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_col19 <- read_csv(glue("{datos}/suicidios_edad_col_2019-2020.csv"))
def_col <- rbind (def_col, def_col19)

def_col <- def_col %>% left_join(tot, by = "time") %>% mutate(value = (as.numeric(value)/poblacion)*100000) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

write_csv(def_col, glue("{datos}/base_suicidios_edad_col_2010-2020.csv"))
...
dpto <- def_dpto %>% filter(dpto == "TOTAL") %>% filter(na_2 != "TOTAL") %>%
  ...
  mutate(id_data = 19, variable = "suicidios", value = (as.numeric(value)/poblacion)*100000,
         nivel_value = glue("{as.numeric(id_d)}"), id_nivel = "dpto", variable = "suicidios") %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_dpto19 <- read_csv(glue("{datos}/suicidios_dpto_2019-2020.csv"))

nac_dpto <- rbind (dpto, def_dpto19)

write_csv(nac_dpto, glue("{datos}/base_suicidios_dpto_2010-2020.csv"))
...
dpto <- def_dpto %>% filter(dpto == "TOTAL") %>% filter(na_2 != "TOTAL") %>%
  pivot_longer(cols = starts_with("gen_"), names_to = "gen", values_to = "suicidios") %>%
  mutate(cod_dpto = as.numeric(id_d), year = time) %>% left_join(tot, by = c("cod_dpto", "year")) %>%
  mutate(id_data = 19, variable = "suicidios", id_data = 19, variable = "suicidios",
         gen = as.numeric(gsub("gen_", "", gen)), value = (as.numeric(suicidios)/poblacion)*100000,
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         id_nivel = "dpto_gen", id_time = 1, nivel_value = glue("{as.numeric(id_d)}_{gen}")) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_dpto19 <- read_csv(glue("{datos}/suicidios_genero_dpto_2019-2020.csv"))

dpto <- rbind (dpto, def_dpto19)

write_csv(dpto, glue("{datos}/base_suicidios_genero_dpto_2010-2020.csv"))
...
dpto <- def_dpto %>% filter(dpto == "TOTAL") %>% 
  mutate(cod_dpto = as.numeric(id_d), year = time) %>% left_join(tot, by = c("cod_dpto", "year")) %>%
  mutate(id_data = 19, variable = "suicidios", id_data = 19, variable = "suicidios",
         value = (as.numeric(suicidios)/poblacion)*100000,
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         nivel_label = glue("{nom_dpto}_{edad}"), id_nivel = "dpto_edad", id_time = 1, 
         nivel_value = glue("{as.numeric(id_d)}_{edad}"), nivel_label = gsub(" ", "_", nivel_label)) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, nivel_label, value)

def_dpto19 <- read_csv(glue("{datos}/suicidios_edad_dpto_2019-2020.csv"))

dpto <- rbind (dpto, def_dpto19)

write_csv(dpto, glue("{datos}/base_suicidios_edad_dpto_2010-2020.csv"))
...
dpto <- def_dpto %>% mutate(nivel_value = glue("{id_d}"), id_nivel = "mpio", variable = "suicidios") %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_dpto19 <- read_csv(glue("{datos}/base_suicidios_mpio_2019-2020.csv"))

nac_dpto <- rbind (dpto, def_dpto19)

write_csv(nac_dpto, glue("{datos}/base_suicidios_mpio_2010-2020.csv"))

rm(cod_dpto, def_dpto19, dpto, nac_dpto)

#Suicidios - género----

dpto <- def_dpto %>% pivot_longer(cols = starts_with("gen_"), names_to = "gen", values_to = "suicidios") %>%
  mutate(id_data = 19, variable = "suicidios", gen = as.numeric(gsub("gen_", "", gen)),
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         id_nivel = "mpio_gen", id_time = 1, nivel_value = glue("{id_d}_{gen}"), value = suicidios) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_dpto19 <- read_csv(glue("{datos}/base_suicidios_genero_mpio_2019-2020.csv"))

dpto <- rbind (dpto, def_dpto19)

write_csv(dpto, glue("{datos}/base_suicidios_genero_mpio_2010-2020.csv"))
...
def_dpto <- def_dpto %>% mutate(id_data = 19, variable = "suicidios",
         value_label = "Fallecidos por lesiones autoinfligidas intencionalmente (suicidios)",
         id_nivel = "mpio_edad", id_time = 1, nivel_value = glue("{id_d}_{edad}"), value = suicidios) %>% 
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value)

def_dpto19 <- read_csv(glue("{datos}/base_suicidios_edad_mpio_2019-2020.csv"))

def_dpto <- rbind (def_dpto, def_dpto19)

write_csv(def_dpto, glue("{datos}/base_suicidios_edad_mpio_2010-2020.csv"))
