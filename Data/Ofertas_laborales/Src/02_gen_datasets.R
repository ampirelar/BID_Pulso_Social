...
# packageList<-c("tidyverse","textclean","scales","stringr","plyr","readxl")
...
# ---- COMPUTRABAJO ----
#~~~~~~~~~~~~~~~~~~~~~~~~#
codes <- list('educacion basica primaria'='basica primaria (1 - 5)', 'educacion basica secundaria'='basica secundaria (6 - 9)',
              'educacion media'='media (10 - 13)', 'carrera tecnica'='tecnologica',
              'carrera tecnologica'='tecnologica', 'carrera profesional'='universitaria',
              'maestria'='maestria', 'especializacion'='especializacion', 'doctorado'='doctorado')

files_all <- list.files(pattern="*.rds", full.names=TRUE, paste0(path_data,"output_cleaned_computrabajo"))
computrabajo <- ldply(files_all, readRDS) %>%
  distinct() %>% mutate(month_year = date_format(format = "%Y%b")(date)) %>%
  #Eliminate 0s after comma and keep numbers
  mutate(wage_nummeric = gsub("(.*),.*", "\\1", wage)) %>%
  mutate(wage_nummeric = as.numeric(gsub("\\D+", "",wage_nummeric))) %>%
  mutate(wage = cut(wage_nummeric, breaks = c(0,1000000,seq(1500000,4500000, by = 500000),
                                              5500000,6000000,8000000,10000000,12500000,
                                              15000000,18000000,21000000, Inf),
                    labels = c("menos de $1 millon", "$1 a $1,5 millones",
  ...
  dplyr::rename(working_day = contract) %>%
  mutate(requirements = trim(tolower(requirements)),
         contract = gsub("a termino ","",gsub("contrato ","",trim(tolower(working_hours))))) %>%
  mutate(contract = ifelse(contract == "civil por prestacion de servicios","prestacion de servicios", contract)) %>%
  mutate(contract = ifelse(contract == "definid","definido", contract)) %>%
  mutate(contract = ifelse(contract == "de obra o labor","por obra o labor", contract)) %>%
  mutate(contract = ifelse(contract == "fijo","definido", contract),
         location=ifelse(location=="bogota, d.c.","bogota",
                         ifelse(location=="cartagena de indias","cartagena",location))) %>%
  #Extract required education
  mutate(education_level = gsub(".*educacion minima: \\s*| -new_req: .*", "", requirements)) %>%
  mutate(education_level = gsub(".* / ","", education_level)) %>%
  mutate(education_level = dplyr::recode(education_level, !!!codes)) %>%
  mutate(education_level = ordered(education_level, levels=c("preescolar","basica primaria (1 - 5)", "basica secundaria (6 - 9)",
                                                             " media (10 - 13)", "tecnologica", "universitaria","especializacion",
                                                             "maestria","doctorado"))) %>%
  #Extract required experience, age, language
  mutate(experience = as.numeric(ifelse(grepl("anos de experiencia", requirements), gsub(".*anos de experiencia: \\s*| -new_req: .*", "", requirements), NA)),
         age = gsub("entre ","",(ifelse(grepl("edad:", requirements), gsub(".*edad: \\s*| -new_req: .*", "", requirements), ""))),
         languages = ifelse(grepl("idiomas:", requirements), gsub(".*idiomas: \\s*| -new_req: .*", "", requirements), NA),
         traveling = ifelse(grepl("disponibilidad de viajar:", requirements), gsub(".*disponibilidad de viajar: \\s*| -new_req: .*", "", requirements), NA),
         knowledge = ifelse(grepl("conocimientos:", requirements), gsub(".*conocimientos: \\s*| -new_req: .*", "", requirements), NA),
         migrate = ifelse(grepl("disponibilidad de cambio de residencia:", requirements), gsub(".*disponibilidad de cambio de residencia: \\s*| -new_req:.*", "", requirements), NA)) %>%
  mutate(number_languages = ifelse(!is.na(languages), sapply(strsplit(gsub(",","", languages), " "), length),0)) %>%
  mutate(covid = ifelse(date >= parse_date(as.character("2020-03-06"), "%Y-%m-%d", locale = locale("es")), 1,0)) %>%
  mutate_if(is.character, tolower) %>% mutate_if(is.character, trim) %>%
  #Do they mention teletrabajo?
  mutate(teletrabajo = ifelse(grepl("teletrabajo|trabajo a distancia", title), 1,
                       ifelse(grepl("teletrabajo|trabajo a distancia", requirements),1,
                              ifelse(grepl("teletrabajo|trabajo a distancia", description),1,0)))) %>%
  #Add dates
  mutate(year_week = paste(year, week_number),
         day_of_year = date_format(format = "%j")(date))

computrabajo2 <- computrabajo %>%
  dplyr::distinct(url, .keep_all= TRUE) %>%
  # dplyr::distinct(description, month_year, .keep_all = T)
  dplyr::distinct(title, location, company, description, month_year, .keep_all = TRUE)

max(computrabajo2$date, na.remove = TRUE)
colSums(is.na(computrabajo2))

saveRDS(computrabajo2, paste0(path_new_data,"computrabajo.rds"))
