...
# Procesamiento de datos de Asistencia escolar 13-17 años
#-------------------------------------------------------#
...
educ <- educ_ori %>%
  dplyr::select(zona, cod_dpto, year, edad_13_17, asistencia_edu_sec, totalp3, asistencia_edu_sec_per) %>%
  mutate(asistencia_esc = asistencia_edu_sec/totalp3, zona = ifelse(zona > 1, 2, zona))
...
esc <- educ %>%
  ...
  summarise(asistencia_edu_sec = sum(asistencia_edu_sec), totalp3 = sum(totalp3)) %>%
  ungroup() %>%
...
esc_dpto <- esc %>%
  ...
  summarise(asistencia_edu_sec = sum(asistencia_edu_sec), total = sum(totalp3)) %>%
  ungroup() %>%
  mutate(asistencia_esc = 100*(asistencia_edu_sec/total))

# Organizar base datos
esc_dpto <- esc_dpto %>% 
  mutate(id_data = 2, variable = "asistencia_escolar_13-17", value_label = "Asistencia escolar de 13 a 17 años (%)", 
...
# Exportar base
write_csv(esc_dpto, glue("{datos}/base_asistencia_escolar_13-17_dpto_2010-2020.csv"))
...
# Exportar base
write_csv(esc_zona, glue("{datos}/base_asistencia_escolar_13-17_zonas_2010-2020.csv"))
...
# Zonas: 1 Cabecera; 2 Centros poblados; 3 rural dispersa
educ <- educ_ori %>%
  dplyr::select(zona, cod_dpto, year, quin, asistencia_edu_sec, totalp3, asistencia_edu_sec_per) %>%
  mutate(asistencia_esc = asistencia_edu_sec/totalp3, zona = ifelse(zona > 1, 2, zona))
...
sec_dpto <- sec %>%
  ...
  summarise(asistencia_edu_sec = sum(asistencia_edu_sec), total = sum(total)) %>%
  ungroup() %>%
  mutate(asistencia_esc = 100*(asistencia_edu_sec/total),
         asistencia_esc = ifelse(asistencia_edu_sec + total == 0, NA, asistencia_esc))

# Organizar base datos
sec_dpto <- sec_dpto %>% 
  mutate(id_data = 2, variable = "asistencia_escolar_13-17", 
         value_label = glue("Asistencia escolar de 13 a 17 años por quintil de ingreso (%) - quintil {quin}"), 
         id_nivel = "dpto", id_time = 1, time = year, value = asistencia_esc, nivel_value = cod_dpto) %>%
  select(id_data, variable, id_nivel, nivel_value, id_time, time, value_label, value) %>%
  drop_na(nivel_value) %>%
  arrange(nivel_value, time)
...
# Exportar base
# write_csv(sec_dpto, glue("{datos}/base_asistencia_escolar_13-17_dpto_quintil_2010-2020.csv"))
...
# Exportar base
# write_csv(esc_zona, glue("{datos}/base_asistencia_escolar_13-17_zonas_quintil_2010-2020.csv"))
...
lapply(unique(sec_dpto$value_label), function(x){
  ...
  # Exportar base
  write_csv(dptos_df, glue("{datos}/base_asistencia_escolar_13-17_dpto_quintil_{quintil}_2010-2020.csv"))
  write_csv(zonas_df, glue("{datos}/base_asistencia_escolar_13-17_zonas_quintil_{quintil}_2010-2020.csv"))
