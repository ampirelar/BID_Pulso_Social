#-------------------------------------------------------#
# Pulso Social BID ----
# Ultima fecha de modificacion: 27 agosto, 2021
...
# Emparejamos base de datos con codigos departamento

if(unique(data$id_nivel) == "dpto"){
  data <- data %>% left_join(nom_dpto, by = "nivel_value")
} 

if(grepl("gen", unique(data$id_nivel))){
  data <- data %>%
...
xlab_time <- ifelse(unique(data$id_time) == 1, "Año",
                    ifelse(unique(data$id_time == 2, "Mes", 
                                  ifelse(unique(data$time) == 3, "Día", NULL))))
xlab_terr <- ifelse(grepl("dpto", unique(data$id_nivel)), "Departamento",
                    ifelse(grepl("mpio", unique(data$id_nivel)), "Municipio", NULL))
ylab <- unique(data$value_label)
